...
const deleteLocation = async (event) => {
  ...
  // Delete the location and its associated weather data
  const deleteWeatherQuery = {
    text: 'DELETE FROM weather WHERE location_id = $1 AND user_uuid = $2',
    values: [locationId, userId]
  };
  await pool.query(deleteWeatherQuery);

  const deleteLocationQuery = {
    text: 'DELETE FROM locations WHERE id = $1 AND user_uuid = $2',
    values: [locationId, userId]
  };
  await pool.query(deleteLocationQuery);
